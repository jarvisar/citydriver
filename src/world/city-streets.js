...
export function buildStreets(c) {
  ...
  c.surface(B / 2, R - .02, south.halfWidth / 2, B, .04, south.halfWidth, '#ffffff', 'road');
  c.surface(B / 2, R - .02, B - north.halfWidth / 2, B, .04, north.halfWidth, '#ffffff', 'road');
  c.surface(west.halfWidth / 2, R - .02, middleS, west.halfWidth, .04, length, '#ffffff', 'road');
  c.surface(B - east.halfWidth / 2, R - .02, middleS, east.halfWidth, .04, length, '#ffffff', 'road');
  for (const [axis, profile] of [['north', west], ['east', south]]) {
    const along = (p, across, width, depth, color, y = R + .017, h = .018) => {
      if (axis === 'north') c.surface(across, y, p, width, h, depth, color);
      else c.surface(p, y, across, depth, h, width, color);
    };
    ...
    if (profile.kind === 'boulevard') {
      ...
      const planted = axis === 'north' ? !c.plan.rivers.east : !c.plan.rivers.north;
      if (planted) {
  ...
  for (const [x, nx, dx] of [[0, west, 1], [B, east, -1]]) for (const [s, ex, ds] of [[0, south, 1], [B, north, -1]]) {
    for (let p = .9; p < nx.halfWidth - .4; p += 1.7) c.surface(x + dx * p, R + .017, s + ds * (ex.halfWidth + 3), .9, .018, 2.8, '#deddd0');
    for (let p = .9; p < ex.halfWidth - .4; p += 1.7) c.surface(x + dx * (nx.halfWidth + 3), R + .017, s + ds * p, 2.8, .018, .9, '#deddd0');
  }
  const approaches = [
  ...
  for (const a of approaches) {
    ...
    if (a.axis === 'north') c.surface(lane, R + .02, a.stop, paintWidth, .02, .3, '#e1dfce');
    else c.surface(a.stop, R + .02, lane, .3, .02, paintWidth, '#e1dfce');
