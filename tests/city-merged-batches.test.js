...
function buildWithBatches(ix, iz, materials) {
  const chunk = new CitydriverChunk(ix, iz, materials, false, true);
  let batches = null;
...
test('merged furniture reproduces every instance exactly as the instancing shader draws it', () => {
  ...
  try {
    for (const [ix, iz] of [[0, 0], [1, 0], [3, 0], [-2, -3], [2, 5], [-4, 1]]) {
      const { chunk, batches } = buildWithBatches(ix, iz, world.materials);
...
test('merging cuts each block\'s draws without growing its memory much', () => {
  ...
  try {
    for (const [ix, iz] of [[0, 0], [1, 0], [-2, -3]]) {
      ...
      assert.ok(chunk.group.children.length <= batchCount - 4, 'at least four fewer draws per pass');
      let bytes = 0;
