...
const WALLS = ['#98756b', '#ad8e78', '#baae92', '#778b96', '#a6b2ad', '#8f979d', '#b49a88', '#697e8b', '#bfafa0', '#896f68'];
...
export class CitydriverChunk {
  constructor(ix, iz, materials, distant = false) {
    ...
    if (!distant) { this.buildFurniture(); this.finish(); }
  }
  item(key, geometry, material, p, scale = [1, 1, 1], color = '#ffffff', yaw = 0) {
    if (!this.batches.has(key)) this.batches.set(key, { geometry, material, items: [] });
    this.batches.get(key).items.push({ p, scale, color, yaw });
  }
  box(x, y, s, width, height, depth, color, kind = 'solid', yaw = 0) {
    this.item(kind, boxGeometry, this.materials[kind], [x, y, -s], [width, height, depth], color, yaw);
  }
  ...
  buildBuildings() {
    const random = this.random, district = this.plan.district;
    for (const x of [34, 78]) for (const s of [34, 78]) {
      const width = 28 + random() * 9, depth = 28 + random() * 9;
      const floors = district === 'Midtown' ? 7 + Math.floor(random() * 7)
        : district === 'Warehouse district' ? 2 + Math.floor(random() * 3) : 3 + Math.floor(random() * 5);
      this.buildBuilding(x, s, width, depth, floors, pick(WALLS, random), pick(AWNINGS, random));
    }
  ...
  buildRiver() {
    ...
    for (const s of [10.6, 101.4]) {
      ...
      for (let x = 33; x < 78; x += 11.5) {
        ...
        if (!this.batches.has('bridge-braces')) this.batches.set('bridge-braces', { geometry: boxGeometry, material: this.materials.solid, items: [] });
        this.batches.get('bridge-braces').items.push({ p, scale: [length, .32, .4], color: iron, roll: Math.atan2((Math.round((x - 33) / 11.5) % 2 ? -1 : 1) * 5.4, 11.5) });
      }
    }
