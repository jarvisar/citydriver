...
// Open-wheel racer. Chooser-only, like the GT, so traffic keeps an
// ordinary-looking fleet.
export const FORMULA_SHAPE = {
  name: 'formula', width: 1.9, length: 5.2,
  // Read by the chooser art like a road car's measurements.
...
export function createFormulaCar(entry) {
  ...
  // Pulling in one end of a box keeps the nose and engine cover faceted,
  // matching the road cars' glass.
  ...
  // Carbon pylon, so it does not read as a painted fin from behind.
  ...
  // Rain light keeps the racer visible at night.
