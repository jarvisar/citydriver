...
`npm test` covers the goal draw, goal completion and banking, career totals, records, promotions, livery unlocks and the group route penalties. With a dev server, `npm run test:taxi` and `npm run test:fleet` exercise the results screen, the fleet dialog and goal banking in the browser.
