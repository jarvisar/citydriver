...
try {
  ...
  assert.equal(await page.locator('button[data-place-type]').count(), 0);
  ...
  await page.click('#city-map-toggle');
  assert.equal(await page.locator('#next-city-stop').isVisible(), false);
  assert.equal(await page.locator('.city-guide-stop').isVisible(), false);
  ...
  assert.equal(await mobile.locator('button[data-place-type]').count(), 0);
