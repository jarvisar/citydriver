...
export function createClassicCar(entry = carEntry(DEFAULT_CAR)) {
  ...
  // Merge static body parts by material. The plate is excluded because it moves
  // for the spare tire.
  ...
  // One accessory per journey trim; applyTrim shows the matching one.
  ...
  // Route changes reuse this model rather than rebuilding it. A fixed trim
  // ignores the route; garage paint overrides both.
...
const STEEP = 1.2;
// Radius (m) around the car's centre that must be sound ground: a little over
// half its length, so no corner hangs over a quay or cliff whichever way it faces.
const FOOTING = 2.5;
export const impassable = ground => ground.blocked;
// Decay rates (1/s) for collision knock: the slide is gone in about half a
// second, the spin a little sooner.
const SLIDE_GRIP = 5, SPIN_GRIP = 8;
// Maximum body lean, in radians.
const LEAN = .105;
// A handbrake tap leaves the slide available for this long, so the button and
// the steering can be pressed in either order.
const DRIFT_ARM = .3;
// Cap (m) on render extrapolation, so a collision correction cannot throw the
// body ahead of itself.
const LEAD_REACH = .5;
...
export class DrivingController {
  constructor(route = citydriverRoute, state = {}, carId = DEFAULT_CAR, paint = null) {
    ...
    this.audioTelemetry = { speed: 0, throttle: 0, brake: 0, offRoad: 0, steer: 0, handbrake: 0, slip: 0, impact: 0, impactSerial: 0 };
  ...
  // Swapping cars keeps position and speed. Paint belongs to the new car, so it
  // is passed in rather than kept.
  setCar(id, { rebuild = true, paint = null } = {}) {
    ...
    // Driver view sits just behind the windshield. Traffic-shaped cabins slope
    // back .24 m at the top of the glass; other bodies give their own `eye`.
  ...
  // A garage colour, or null for the factory finish.
  setPaint(color) { this.paintColor = color ?? null; this.updatePaint(); }
  updatePaint(dt = 0) {
    if (this.rainbow) {
      // Cycles the hue without touching the garage paint.
  ...
  // Actual velocity: the car's own drive plus any collision knock.
  ...
  // Applies a world-space move (m) in road (s, u) coordinates.
  shift(dx, dz) {
  ...
  // Knock decays exponentially and snaps to zero once small.
  carryKnock(dt) {
  ...
  // This car takes its mass-weighted share of the blow (see impact.js). The
  // part along its heading changes speed but never reverses it through zero;
  // the rest becomes knock, which carryKnock wears off.
  resolveTrafficCollision(dx, dz, dvx = 0, dvz = 0, spin = 0) {
    ...
    const cos = Math.cos(this.slideHeading ?? this.heading), sin = Math.sin(this.slideHeading ?? this.heading), along = dvx * sin - dvz * cos, across = dvx * cos + dvz * sin;
  ...
  // Scenery does not move. The car is pushed out along the contact normal,
  // keeps only the speed along the face, and is turned toward it so a glancing
  // blow slides off a wall instead of stopping dead.
  resolveSceneryCollision(nx, nz, depth, dt) {
    const heading = this.slideHeading ?? this.heading, direction = Math.sign(this.speed);
    const fx = Math.sin(heading) * direction, fz = -Math.cos(heading) * direction;
    ...
    // Also cancel any knock driving into the face.
    const into = this.knock.x * nx + this.knock.z * nz;
    if (into < 0) { this.knock.x -= into * nx; this.knock.z -= into * nz; }
    // Off the road (s, u) is not a rigid frame, so map the push through the
    // route's own position function rather than the road angle.
  ...
  // Ground height and slope along/across the road over about a wheelbase and a
  // track. In free driving, also marks it blocked on water or a cliff face, or
  // with either within FOOTING. The road itself is always sound.
  ground(s, u) {
  ...
  // `lead` is how far (0..1 of a step) this display frame falls past the last
  // simulation step. Extrapolating by it shows the car where it is now;
  // interpolating between the last two steps would add a step of visible lag.
  ...
  update(dt, input) {
    ...
    // No slow input filter: the precision curve shapes the target, and a
    // keypress reaches full lock in about 30 ms (release and countersteer faster).
    ...
    // 0 on the road, 1 on open ground, ramped over about half a car width so
    // two wheels on the verge cost a fraction of leaving entirely. It drives
    // resistance, steering and sound alike. The ramp ends by 5.9 m because the
    // alpine road's shoulder is only 6.3 m wide.
    const looseness = this.route.looseness?.(this.s, this.u) ?? clamp((Math.abs(this.u) - 4.8) / 1.1, 0, 1);
    // Off-road resistance that full throttle balances at stats.offRoad, plus
    // more above it so arriving fast bleeds off over a second or so. It ramps
    // up with speed: full drag at a standstill can exceed reverse torque and
    // trap the car in the grass.
    ...
    // Lost turn-in is what makes grass feel like grass. Capped at a quarter so
    // the car stays recoverable; with the wheel centred, lane assist still
    // points it back at the road.
    ...
    // Driver demand before drag and surface losses. Weight transfer reads this,
    // not the net total, so a verge does not load the front tires like a brake.
    let pedals = acceleration;
    // Sliding scrubs a little speed: enough that a slide costs something, not
    // so much that its tighter line never pays. Full throttle can carry the
    // slide; lifting lets the tires catch.
    ...
    // Weight transfer as a share of this car's acceleration or braking. Measured
    // along the direction of travel, so the brake used as a reverse throttle
    // lifts the nose. Eases in over about a tenth of a second.
    ...
    // Free driving has no roadside limits, so water and cliffs stop the car.
    // Keep whichever axis of the move stays on firm ground so it slides along
    // a shore instead of sticking; a car already on impassable ground may leave.
    ...
    // Lean and dive use the same yaw and acceleration as the physics.
    ...
    // Chase-camera speed effect: 0 below 40% of top speed, 1 at top speed.
    this.car.userData.speedRush = clamp((Math.abs(this.speed) / stats.topSpeed - .4) / .6, 0, 1);
    // Read-only audio telemetry; sound never feeds back into the physics.
